import { DiscountSvg, MailSvg, ServiceBoxSvg } from "@/data/svg/flight-svg";

export const cityData = [
  {
    id: 1,
    place: "Kabul, Afghanistan",
    airport: "Hamid Karzai International Airport",
    button: "KBL",
  },
  {
    id: 2,
    place: "Tirana, Albania",
    airport: "Tirana International Airport",
    button: "TIA",
  },
  {
    id: 3,
    place: "Algiers, Algeria",
    airport: "Houari Boumediene Airport",
    button: "ALG",
  },
  {
    id: 4,
    place: "Andorra la Vella, Andorra",
    airport: "Andorra–La Seu d'Urgell Airport",
    button: "LEU",
  },
  {
    id: 5,
    place: "Luanda, Angola",
    airport: "Quatro de Fevereiro Airport",
    button: "LAD",
  },
  {
    id: 6,
    place: "Antigua, Antigua and Barbuda",
    airport: "V.C. Bird International Airport",
    button: "ANU",
  },
  {
    id: 7,
    place: "Buenos Aires, Argentina",
    airport: "Ministro Pistarini International Airport",
    button: "EZE",
  },
  {
    id: 8,
    place: "Yerevan, Armenia",
    airport: "Zvartnots International Airport",
    button: "EVN",
  },
  {
    id: 9,
    place: "Sydney, Australia",
    airport: "Sydney Kingsford Smith Airport",
    button: "SYD",
  },
  {
    id: 10,
    place: "Vienna, Austria",
    airport: "Vienna International Airport",
    button: "VIE",
  },
  {
    id: 11,
    place: "Baku, Azerbaijan",
    airport: "Heydar Aliyev International Airport",
    button: "GYD",
  },
  {
    id: 12,
    place: "Nassau, Bahamas",
    airport: "Lynden Pindling International Airport",
    button: "NAS",
  },
  {
    id: 13,
    place: "Manama, Bahrain",
    airport: "Bahrain International Airport",
    button: "BAH",
  },
  {
    id: 14,
    place: "Dhaka, Bangladesh",
    airport: "Hazrat Shahjalal International Airport",
    button: "DAC",
  },
  {
    id: 15,
    place: "Bridgetown, Barbados",
    airport: "Grantley Adams International Airport",
    button: "BGI",
  },
  {
    id: 16,
    place: "Minsk, Belarus",
    airport: "Minsk National Airport",
    button: "MSQ",
  },
  {
    id: 17,
    place: "Brussels, Belgium",
    airport: "Brussels Airport",
    button: "BRU",
  },
  {
    id: 18,
    place: "Belize City, Belize",
    airport: "Philip S. W. Goldson International Airport",
    button: "BZE",
  },
  {
    id: 19,
    place: "Cotonou, Benin",
    airport: "Cadjehoun Airport",
    button: "COO",
  },
  {
    id: 20,
    place: "Thimphu, Bhutan",
    airport: "Paro Airport",
    button: "PBH",
  },
  {
    id: 21,
    place: "Sucre, Bolivia",
    airport: "Juancito Pinto Airport",
    button: "SRE",
  },
  {
    id: 22,
    place: "Sarajevo, Bosnia and Herzegovina",
    airport: "Sarajevo International Airport",
    button: "SJJ",
  },
  {
    id: 23,
    place: "Gaborone, Botswana",
    airport: "Sir Seretse Khama International Airport",
    button: "GBE",
  },
  {
    id: 24,
    place: "Brasília, Brazil",
    airport: "Brasília International Airport",
    button: "BSB",
  },
  {
    id: 25,
    place: "Bandar Seri Begawan, Brunei",
    airport: "Brunei International Airport",
    button: "BWN",
  },
  {
    id: 26,
    place: "Sofia, Bulgaria",
    airport: "Sofia Airport",
    button: "SOF",
  },
  {
    id: 27,
    place: "Ouagadougou, Burkina Faso",
    airport: "Ouagadougou Airport",
    button: "OUA",
  },
  {
    id: 28,
    place: "Bujumbura, Burundi",
    airport: "Bujumbura International Airport",
    button: "BJM",
  },
  {
    id: 29,
    place: "Praia, Cabo Verde",
    airport: "Nelson Mandela International Airport",
    button: "RAI",
  },
  {
    id: 30,
    place: "Phnom Penh, Cambodia",
    airport: "Phnom Penh International Airport",
    button: "PNH",
  },
  {
    id: 31,
    place: "Yaoundé, Cameroon",
    airport: "Yaoundé Nsimalen International Airport",
    button: "NSI",
  },
  {
    id: 32,
    place: "Toronto, Canada",
    airport: "Toronto Pearson International Airport",
    button: "YYZ",
  },
  {
    id: 33,
    place: "Bangui, Central African Republic",
    airport: "Bangui M'Poko International Airport",
    button: "BGF",
  },
  {
    id: 34,
    place: "N'Djamena, Chad",
    airport: "N'Djamena International Airport",
    button: "NDJ",
  },
  {
    id: 35,
    place: "Santiago, Chile",
    airport: "Comodoro Arturo Merino Benítez International Airport",
    button: "SCL",
  },
  {
    id: 36,
    place: "Beijing, China",
    airport: "Beijing Capital International Airport",
    button: "PEK",
  },
  {
    id: 37,
    place: "Bogotá, Colombia",
    airport: "El Dorado International Airport",
    button: "BOG",
  },
  {
    id: 38,
    place: "Moroni, Comoros",
    airport: "Moroni International Airport",
    button: "HAH",
  },
  {
    id: 39,
    place: "Kinshasa, Congo, Democratic Republic of the",
    airport: "N'djili Airport",
    button: "FIH",
  },
  {
    id: 40,
    place: "Brazzaville, Congo, Republic of the",
    airport: "Maya-Maya Airport",
    button: "BZV",
  },
  {
    id: 41,
    place: "San José, Costa Rica",
    airport: "Juan Santamaría International Airport",
    button: "SJO",
  },
  {
    id: 42,
    place: "Zagreb, Croatia",
    airport: "Zagreb Airport",
    button: "ZAG",
  },
  {
    id: 43,
    place: "Havana, Cuba",
    airport: "José Martí International Airport",
    button: "HAV",
  },
  {
    id: 44,
    place: "Nicosia, Cyprus",
    airport: "Larnaca International Airport",
    button: "LCA",
  },
  {
    id: 45,
    place: "Prague, Czech Republic",
    airport: "Václav Havel Airport Prague",
    button: "PRG",
  },
  {
    id: 46,
    place: "Copenhagen, Denmark",
    airport: "Copenhagen Airport",
    button: "CPH",
  },
  {
    id: 47,
    place: "Djibouti, Djibouti",
    airport: "Djibouti-Ambouli International Airport",
    button: "JIB",
  },
  {
    id: 48,
    place: "Roseau, Dominica",
    airport: "Douglas-Charles Airport",
    button: "DOM",
  },
  {
    id: 49,
    place: "Santo Domingo, Dominican Republic",
    airport: "Las Américas International Airport",
    button: "SDQ",
  },
  {
    id: 50,
    place: "Quito, Ecuador",
    airport: "Mariscal Sucre International Airport",
    button: "UIO",
  },
  {
    id: 51,
    place: "Cairo, Egypt",
    airport: "Cairo International Airport",
    button: "CAI",
  },
  {
    id: 52,
    place: "San Salvador, El Salvador",
    airport: "El Salvador International Airport",
    button: "SAL",
  },
  {
    id: 53,
    place: "Malabo, Equatorial Guinea",
    airport: "Malabo International Airport",
    button: "SSG",
  },
  {
    id: 54,
    place: "Asmara, Eritrea",
    airport: "Asmara International Airport",
    button: "ASM",
  },
  {
    id: 55,
    place: "Tallinn, Estonia",
    airport: "Tallinn Airport",
    button: "TLL",
  },
  {
    id: 56,
    place: "Mbabane, Eswatini",
    airport: "King Mswati III International Airport",
    button: "MTS",
  },
  {
    id: 57,
    place: "Addis Ababa, Ethiopia",
    airport: "Addis Ababa Bole International Airport",
    button: "ADD",
  },
  {
    id: 58,
    place: "Nadi, Fiji",
    airport: "Nadi International Airport",
    button: "NAN",
  },
  {
    id: 59,
    place: "Helsinki, Finland",
    airport: "Helsinki-Vantaa Airport",
    button: "HEL",
  },
  {
    id: 60,
    place: "Paris, France",
    airport: "Charles de Gaulle Airport",
    button: "CDG",
  },
  {
    id: 61,
    place: "Libreville, Gabon",
    airport: "Leon Mba International Airport",
    button: "LBV",
  },
  {
    id: 62,
    place: "Banjul, Gambia",
    airport: "Banjul International Airport",
    button: "BJL",
  },
  {
    id: 63,
    place: "Tbilisi, Georgia",
    airport: "Tbilisi International Airport",
    button: "TBS",
  },
  {
    id: 64,
    place: "Berlin, Germany",
    airport: "Berlin Brandenburg Airport",
    button: "BER",
  },
  {
    id: 65,
    place: "Accra, Ghana",
    airport: "Kotoka International Airport",
    button: "ACC",
  },
  {
    id: 66,
    place: "Athens, Greece",
    airport: "Athens International Airport",
    button: "ATH",
  },
  {
    id: 67,
    place: "Helsinki, Greenland",
    airport: "Nuuk Airport",
    button: "NUK",
  },
  {
    id: 68,
    place: "Guatemala City, Guatemala",
    airport: "La Aurora International Airport",
    button: "GUA",
  },
  {
    id: 69,
    place: "Conakry, Guinea",
    airport: "Conakry International Airport",
    button: "CKY",
  },
  {
    id: 70,
    place: "Bissau, Guinea-Bissau",
    airport: "Osvaldo Vieira International Airport",
    button: "OXB",
  },
  {
    id: 71,
    place: "Georgetown, Guyana",
    airport: "Cheddi Jagan International Airport",
    button: "GEO",
  },
  {
    id: 72,
    place: "Port-au-Prince, Haiti",
    airport: "Toussaint Louverture International Airport",
    button: "PAP",
  },
  {
    id: 73,
    place: "Helsinki, Hungary",
    airport: "Budapest Ferenc Liszt International Airport",
    button: "BUD",
  },
  {
    id: 74,
    place: "Reykjavík, Iceland",
    airport: "Keflavík International Airport",
    button: "KEF",
  },
  {
    id: 75,
    place: "New Delhi, India",
    airport: "Indira Gandhi International Airport",
    button: "DEL",
  },
  {
    id: 76,
    place: "Jakarta, Indonesia",
    airport: "Soekarno-Hatta International Airport",
    button: "CGK",
  },
  {
    id: 77,
    place: "Teheran, Iran",
    airport: "Imam Khomeini International Airport",
    button: "IKA",
  },
  {
    id: 78,
    place: "Baghdad, Iraq",
    airport: "Baghdad International Airport",
    button: "BGW",
  },
  {
    id: 79,
    place: "Dublin, Ireland",
    airport: "Dublin Airport",
    button: "DUB",
  },
  {
    id: 80,
    place: "Rome, Italy",
    airport: "Leonardo da Vinci International Airport",
    button: "FCO",
  },
  {
    id: 81,
    place: "Jerusalem, Israel",
    airport: "Ben Gurion Airport",
    button: "TLV",
  },
  {
    id: 82,
    place: "Athens, Greece",
    airport: "Eleftherios Venizelos Airport",
    button: "ATH",
  },
  {
    id: 83,
    place: "Riyadh, Saudi Arabia",
    airport: "King Khalid International Airport",
    button: "RUH",
  },
  {
    id: 84,
    place: "Singapore, Singapore",
    airport: "Singapore Changi Airport",
    button: "SIN",
  },
  {
    id: 85,
    place: "Madrid, Spain",
    airport: "Adolfo Suárez Madrid-Barajas Airport",
    button: "MAD",
  },
  {
    id: 86,
    place: "Colombo, Sri Lanka",
    airport: "Bandaranaike International Airport",
    button: "CMB",
  },
  {
    id: 87,
    place: "Khartoum, Sudan",
    airport: "Khartoum International Airport",
    button: "KRT",
  },
  {
    id: 88,
    place: "Dar es Salaam, Tanzania",
    airport: "Julius Nyerere International Airport",
    button: "DAR",
  },
  {
    id: 89,
    place: "Bangkok, Thailand",
    airport: "Suvarnabhumi Airport",
    button: "BKK",
  },
  {
    id: 90,
    place: "Lima, Peru",
    airport: "Lima Jorge Chávez International Airport",
    button: "LIM",
  },
  {
    id: 91,
    place: "Helsinki, Finland",
    airport: "Helsinki-Vantaa Airport",
    button: "HEL",
  },
  {
    id: 92,
    place: "Dubai, United Arab Emirates",
    airport: "Dubai International Airport",
    button: "DXB",
  },
  {
    id: 93,
    place: "London, United Kingdom",
    airport: "Heathrow Airport",
    button: "LHR",
  },
  {
    id: 94,
    place: "New York, United States",
    airport: "John F. Kennedy International Airport",
    button: "JFK",
  },
  {
    id: 95,
    place: "Montevideo, Uruguay",
    airport: "Carrasco International Airport",
    button: "MVD",
  },
  {
    id: 96,
    place: "Vatican City, Vatican City",
    airport: "Ciampino–G. B. Pastine International Airport",
    button: "CIA",
  },
  {
    id: 97,
    place: "Caracas, Venezuela",
    airport: "Simón Bolívar International Airport",
    button: "CCS",
  },
  {
    id: 98,
    place: "Hanoi, Vietnam",
    airport: "Noi Bai International Airport",
    button: "HAN",
  },
  {
    id: 99,
    place: "Sanaa, Yemen",
    airport: "Sanaa International Airport",
    button: "SAH",
  },
  {
    id: 100,
    place: "Zagreb, Croatia",
    airport: "Zagreb Airport",
    button: "ZAG",
  },
  {
    id: 101,
    place: "Sao Tome, Sao Tome and Principe",
    airport: "São Tomé International Airport",
    button: "TMS",
  },
  {
    id: 102,
    place: "Lusaka, Zambia",
    airport: "Kenneth Kaunda International Airport",
    button: "LUN",
  },
  {
    id: 103,
    place: "Harare, Zimbabwe",
    airport: "Robert Gabriel Mugabe International Airport",
    button: "HRE",
  },
  {
    id: 104,
    place: "houston, usa",
    airport: "George Bush Intercontinental Airport",
    button: "IAH",
  },
  {
    id: 105,
    place: "moscow, russia",
    airport: "Sheremetyevo International Airport",
    button: "SVO",
  },
  {
    id: 106,
    place: "rome, italy",
    airport: "Leonardo da Vinci International Airport",
    button: "FCO",
  },
  {
    id: 107,
    place: "dublin, ireland",
    airport: "Dublin Airport",
    button: "DUB",
  },
  {
    id: 108,
    place: "lima, peru",
    airport: "Jorge Chávez International Airport",
    button: "LIM",
  },
  {
    id: 109,
    place: "bogotá, colombia",
    airport: "El Dorado International Airport",
    button: "BOG",
  },
  {
    id: 110,
    place: "são paulo, brazil",
    airport:
      "São Paulo/Guarulhos–Governador André Franco Montoro International Airport",
    button: "GRU",
  },
  {
    id: 111,
    place: "cape town, south africa",
    airport: "Cape Town International Airport",
    button: "CPT",
  },
  {
    id: 112,
    place: "sydney, australia",
    airport: "Sydney Kingsford Smith Airport",
    button: "SYD",
  },
  {
    id: 113,
    place: "cairo, egypt",
    airport: "Cairo International Airport",
    button: "CAI",
  },
  {
    id: 114,
    place: "seoul, south korea",
    airport: "Incheon International Airport",
    button: "ICN",
  },
  {
    id: 115,
    place: "auckland, new zealand",
    airport: "Auckland Airport",
    button: "AKL",
  },
  {
    id: 116,
    place: "johannesburg, south africa",
    airport: "O.R. Tambo International Airport",
    button: "JNB",
  },
  {
    id: 117,
    place: "singapore, singapore",
    airport: "Changi Airport",
    button: "SIN",
  },
  {
    id: 118,
    place: "new delhi, india",
    airport: "Indira Gandhi International Airport",
    button: "DEL",
  },
  {
    id: 119,
    place: "nairobi, kenya",
    airport: "Jomo Kenyatta International Airport",
    button: "NBO",
  },
  {
    id: 120,
    place: "new york, usa",
    airport: "John F. Kennedy International Airport",
    button: "JFK",
  },
  {
    id: 121,
    place: "lagos, nigeria",
    airport: "Murtala Muhammed International Airport",
    button: "LOS",
  },
  {
    id: 122,
    place: "abuja, nigeria",
    airport: "Nnamdi Azikiwe International Airport",
    button: "ABV",
  },
  {
    id: 123,
    place: "port harcourt, nigeria",
    airport: "Port Harcourt International Airport",
    button: "PHC",
  },
  {
    id: 124,
    place: "kano, nigeria",
    airport: "Mallam Aminu Kano International Airport",
    button: "KAN",
  },
];


// export const cityData = [
//   {
//     id: 1,
//     place: "paris, france",
//     airport: "Charles de Gaulle Airport",
//     button: "par",
//   },
//   {
//     id: 2,
//     place: "Dubai, UAE",
//     airport: "Dubai International Airport",
//     button: "DXB",
//   },
//   {
//     id: 3,
//     place: "london",
//     airport: "Heathrow",
//     button: "LHR",
//   },
//   {
//     id: 4,
//     place: "singapore, singapore",
//     airport: "changi Airport",
//     button: "par",
//   },
//   {
//     id: 5,
//     place: "vancouver, canada",
//     airport: "vancouver International Airport",
//     button: "par",
//   },
//   {
//     id: 6,
//     place: "sydney, australia",
//     airport: "rose bay SPB",
//     button: "rse",
//   },
//   {
//     id: 7,
//     place: "kuala lumpur, malaysia",
//     airport: "sentral",
//     button: "xkl",
//   },
// ];

export const destinationData = [
  {
    id: 1,
    // img: "/assets/images/flights/destination/1.jpg",
    img: "/assets/images/flights/destination/paris.png",
    country: "france country",
    place: "paris",
    from: 851,
  },
  {
    id: 2,
    // img: "/assets/images/flights/destination/2.jpg",
    img: "/assets/images/flights/destination/london.png",
    country: "united kingdom",
    place: "london",
    from: 369,
  },
  {
    id: 3,
    img: "/assets/images/flights/destination/zurich.png",
    country: "turkey",
    place: "istanbul",
    from: 256,
  },

  {
    id: 4,
    img: "/assets/images/flights/destination/australia.jpg",
    country: "australia",
    place: "sydney",
    from: 581,
  },
  {
    id: 5,
    img: "/assets/images/flights/destination/bali.jpeg",
    // img: "/assets/images/flights/destination/zurich.png",
    country: "indonesia",
    place: "bali",
    from: 256,
  },
  {
    id: 6,
    img: "/assets/images/flights/destination/maldives.jpg",
    // img: "/assets/images/flights/destination/zurich.png",
    country: "maldives",
    place: "maldives island",
    from: 256,
  },
];

export const airDetailData = [
  {
    id: 1,
    title: "airlines",
    offer: "student offer",
  },
  {
    id: 2,
    title: "airlines",
    offer: "10% bonus",
  },
  {
    id: 3,
    title: "airlines",
    offer: "New offer",
  },
];

export const topDealData = [
  {
    id: 1,
    // image: "/assets/images/flights/destination/4.jpg",
    image: "/assets/images/flights/destination/flight-1.jpg",
    title: "london to dubai",
    discount: "-30%",
  },
  {
    id: 2,
    // image: "/assets/images/flights/destination/5.jpg",
    image: "/assets/images/flights/destination/flight-2.jpg",
    title: "australia to turkey",
    discount: "early bird offer",
    // title: "milan to stockholm",
    // discount: "limited time",
  },
  {
    id: 3,
    // image: "/assets/images/flights/destination/6.jpg",
    image: "/assets/images/flights/destination/flight-3.jpg",
    title: "australia to nigeria",
    // discount: "early bird offer",
    // title: "marid to prague",
    discount: "-30%",
  },
  {
    id: 4,
    // image: "/assets/images/flights/destination/7.jpg",
    image: "/assets/images/flights/destination/flight-4.jpg",
    title: "australia to bali",
    discount: "sunday",
  },
];

export const ticketData = [
  {
    id: 1,
    image: "/assets/images/flights/destination/8.jpg",
    title: "london",
    place: [
      { city: "paris", price: 200 },
      { city: "rome", price: 200 },
      { city: "berlin", price: 200 },
    ],
  },
  {
    id: 2,
    image: "/assets/images/flights/destination/9.jpg",
    title: "budva",
    place: [
      { city: "atlanta", price: 200 },
      { city: "miami", price: 200 },
      { city: "ottawa", price: 400 },
    ],
  },
  {
    id: 3,
    image: "/assets/images/flights/destination/10.jpg",
    title: "venice",
    place: [
      { city: "vienna", price: 500 },
      { city: "madrid", price: 500 },
      { city: "dallas", price: 900 },
    ],
  },
  {
    id: 4,
    image: "/assets/images/flights/destination/9.jpg",
    title: "budva",
    place: [
      { city: "atlanta", price: 800 },
      { city: "miami", price: 800 },
      { city: "ottawa", price: 600 },
    ],
  },
];

export const topDestinationData = [
  {
    img: "/assets/images/flights/logo.png",
    name: "japan",
    childern: [
      {
        country: "australia",
        price: 451,
        airline: "35",
      },
      {
        country: "bahrain",
        price: 610,
        airline: "18 ",
      },
      {
        country: "china",
        price: 621,
        airline: "21",
      },
      {
        country: "egypt",
        price: 480,
        airline: "06",
      },
      {
        country: "india",
        price: 320,
        airline: "31",
      },
      {
        country: "australia",
        price: 451,
        airline: "35",
      },
    ],
  },
  {
    img: "/assets/images/flights/logo2.png",
    name: "emirates",
    childern: [
      {
        country: "brazil",
        price: 451,
        airline: "35",
      },
      {
        country: "denmark",
        price: 610,
        airline: "18 ",
      },
      {
        country: "greece",
        price: 621,
        airline: "21",
      },
      {
        country: "iran",
        price: 480,
        airline: "06",
      },
      {
        country: "japan",
        price: 320,
        airline: "31",
      },
      {
        country: "australia",
        price: 451,
        airline: "35",
      },
      {
        country: "brazil",
        price: 451,
        airline: "35",
      },
      {
        country: "denmark",
        price: 610,
        airline: "18 ",
      },
      {
        country: "greece",
        price: 621,
        airline: "21",
      },
      {
        country: "iran",
        price: 480,
        airline: "06",
      },
      {
        country: "japan",
        price: 320,
        airline: "31",
      },
      {
        country: "australia",
        price: 451,
        airline: "35",
      },
    ],
  },
];

export const flightBlogData = [
  {
    id: 1,
    // img: "/assets/images/flights/blog/1.jpg",
    img: "/assets/images/flights/blog/blog-1.jpg",
    date: "05",
    month: "aug",
    location: "Istanbul, Turkey",
    title: "Discover the World: Your Ultimate Guide to Booking Flights",
    desc: `we believe that your journey should start with the best flight experience possible. Whether you're a seasoned traveler or planning your first trip, our comprehensive guide will help you navigate the ins and outs of booking flights, ensuring a smooth and enjoyable travel experience.`,
  },
  {
    id: 2,
    // img: "/assets/images/flights/blog/2.jpg",
    img: "/assets/images/flights/blog/blog-2.jpg",
    date: "05",
    month: "sept",
    location: "Newyork, USA",
    title: "How to Plan a Budget-Friendly Vacation",
    desc: `Planning a budget-friendly vacation doesn't mean you have to sacrifice quality. By booking in advance, you can take advantage of early deals on flights and accommodations, which significantly cuts down costs. Additionally, traveling during off-peak seasons not only saves money but also provides a more relaxed experience with fewer crowds.`,
  },
  {
    id: 3,
    // img: "/assets/images/flights/blog/3.jpg",
    img: "/assets/images/flights/blog/blog-3.jpg",
    date: "05",
    month: "nov",
    location: "London, UK",
    title: "Solo Travel: Tips for First-Time Adventurers",
    desc: `Solo travel can be an exciting and transformative experience, especially for first-time adventurers. Start small by choosing easy-to-navigate, safe destinations like Japan or New Zealand. This helps build confidence while providing a rewarding travel experience. It's also important to plan ahead, ensuring accommodations and transportation are secured, while leaving room for spontaneous activities.`,
  },
  {
    id: 4,
    img: "/assets/images/flights/blog/blog-4.jpg",
    date: "09",
    month: "nov",
    location: "sydney, australia",
    title: "Health and Wellness Travel: Destinations That Will Rejuvenate You",
    desc: `Health and wellness travel has become a popular way for individuals to rejuvenate both body and mind. Destinations like Bali, Indonesia, are perfect for those seeking spiritual healing and tranquility. With a focus on yoga, meditation, and holistic retreats, Bali provides a serene environment for relaxation and self-care.`,
  },
];

export const subscribeData = [
  {
    id: 1,
    svg: <ServiceBoxSvg />,
    title: "call us",
    contact: "+61435607034",
  },
  {
    id: 2,
    svg: <DiscountSvg />,
    title: "special discount",
    contact: "first booking in 25% off",
  },
  {
    id: 3,
    svg: <MailSvg />,
    title: "newsletter",
    contact: "scrambled it to make.",
  },
];

export const holidayFlightData = [
  {
    id: 1,
    titleImg: "/assets/images/flights/offer/3.jpg",
    place: "emirates",
    package: [
      {
        img: "/assets/images/flights/offer/6.jpg",
        title: "norway",
        city: "Dep: paris, dubai",
        price: 250,
        disc: 200,
      },
      {
        img: "/assets/images/flights/offer/7.jpg",
        title: "finland",
        city: "Dep: paris, dubai",
        price: 250,
        disc: 200,
      },
      {
        img: "/assets/images/flights/offer/8.jpg",
        title: "paris",
        city: "Dep: finland, dubai",
        price: 250,
        disc: 200,
      },
    ],
  },
  {
    id: 2,
    titleImg: "/assets/images/flights/offer/4.jpg",
    place: "american eagle",
    package: [
      {
        img: "/assets/images/flights/offer/9.jpg",
        title: "Eastern USA",
        city: "USA, Canada",
        price: 250,
        disc: 200,
      },
      {
        img: "/assets/images/flights/offer/10.jpg",
        title: "canada escape",
        city: "USA, Canada",
        price: 250,
        disc: 200,
      },
      {
        img: "/assets/images/flights/offer/6.jpg",
        title: "Canadian Rockies",
        city: "USA, Canada",
        price: 250,
        disc: 200,
      },
    ],
  },
  {
    id: 3,
    titleImg: "/assets/images/flights/offer/5.jpg",
    place: "Lufthansa",
    package: [
      {
        img: "/assets/images/flights/offer/8.jpg",
        title: "europe escape",
        city: "Prague Paris Barcelona",
        price: 250,
        disc: 200,
      },
      {
        img: "/assets/images/flights/offer/6.jpg",
        title: "europe dream",
        city: "Prague Paris Barcelona",
        price: 250,
        disc: 200,
      },
      {
        img: "/assets/images/flights/offer/10.jpg",
        title: "Europe Wonder",
        city: "Prague Paris Barcelona",
        price: 250,
        disc: 200,
      },
    ],
  },
  {
    id: 4,
    titleImg: "/assets/images/tour/category/4.jpg",
    place: "america",
    package: [
      {
        img: "/assets/images/flights/offer/9.jpg",
        title: "Eastern USA",
        city: "USA, Canada",
        price: 250,
        disc: 200,
      },
      {
        img: "/assets/images/flights/offer/10.jpg",
        title: "canada escape",
        city: "USA, Canada",
        price: 250,
        disc: 200,
      },
      {
        img: "/assets/images/flights/offer/6.jpg",
        title: "Canadian Rockies",
        city: "USA, Canada",
        price: 250,
        disc: 200,
      },
    ],
  },
];

export const packageData = [
  {
    id: 1,
    img: "/assets/images/flights/offer/1.jpg",
    title: "10% cashback",
    badge: "HDFC bank",
    desc: `Lorem Ipsum is simply dummy text of the printing and setting industry. Lorem Ipsum has been the industry's since....`,
    price: 200,
    disc: 250,
    classIcon: true,
    couponCode: "EDR45",
    link: "/",
  },
  {
    id: 2,
    img: "/assets/images/flights/offer/2.jpg",
    title: "250 discount",
    badge: "paypal",
    desc: `Lorem Ipsum is simply dummy text of the printing and setting industry. Lorem Ipsum has been the industry's since....`,
    price: 240,
    disc: 200,
    classIcon: true,
    couponCode: "EDR45",
    link: "/",
  },
  {
    id: 3,
    img: "/assets/images/flights/offer/1.jpg",
    classIcon: true,
    title: "new year offer",
    badge: "world bank",
    desc: `Lorem Ipsum is simply dummy text of the printing and setting industry. Lorem Ipsum has been the industry's since....`,
    price: 240,
    disc: 200,
    couponCode: "EDR45",
    link: "/",
  },
];

export const blogData = [
  {
    id: 1,
    image: "/assets/images/flights/blog/5.jpg",
    date: "01",
    month: "jan",
    title: "posted by: Julia Holmes",
    desc: "Lorem Ipsum is simply the of the printing and dummy text of typesetting..",
    subDesc: "Lorem Ipsum is simply dummy since..",
  },
  {
    id: 2,
    image: "/assets/images/flights/blog/6.jpg",
    date: "24",
    month: "feb",
    title: "posted by: Julia Holmes",
    desc: "Lorem Ipsum is simply the of the printing and dummy text of typesetting..",
    subDesc: "Lorem Ipsum is simply dummy since..",
  },
  {
    id: 3,
    image: "/assets/images/flights/blog/4.jpg",
    date: "27",
    month: "aug",
    title: "posted by: Julia Holmes",
    desc: "Lorem Ipsum is simply the of the printing and dummy text of typesetting..",
    subDesc: "Lorem Ipsum is simply dummy since..",
  },
];

export const dealData = [
  {
    id: 1,
    title: "rica airlines",
    desc: "student new offer",
  },
  {
    id: 2,
    title: "airlines",
    desc: "10% bonus cashback",
  },
  {
    id: 3,
    title: "rica airlines",
    desc: "student new offer",
  },
  {
    id: 4,
    title: "airlines",
    desc: "10% bonus cashback",
  },
  {
    id: 5,
    title: "rica airlines",
    desc: "student new offer",
  },
];

export const airlineDetail = [
  {
    id: 1,
    title: "Dubai",
    img: "/assets/images/tour/slider/1.jpg",
    label: "Recommended",
    class: "label-offer",
  },
  {
    id: 2,
    title: "abu dhabi",
    img: "/assets/images/tour/slider/2.jpg",
  },
  {
    id: 3,
    title: "dubai",
    img: "/assets/images/tour/slider/6.jpg",
  },
  {
    id: 4,
    title: "sydney",
    img: "/assets/images/tour/slider/4.jpg",
  },
  {
    id: 5,
    title: "spain",
    img: "/assets/images/tour/slider/9.jpg",
    label: "certified",
    class: "label-offer",
  },
];

export const airlineDestinationData = [
  {
    id: 1,
    place: "paris",
    place1: "dubai",
  },
  {
    id: 1,
    place: "dubai",
    place1: "sydney",
  },
  {
    id: 3,
    place: "bali",
    place1: "dubai",
  },
  {
    id: 4,
    place: "newyork",
    place1: "paris",
  },
  {
    id: 5,
    place: "paris",
    place1: "dubai",
  },
  {
    id: 6,
    place: "paris",
    place1: "dubai",
  },
];
